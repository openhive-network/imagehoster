...
const {urlHash, isUploadHash, getKeyFromUrl} = require("./hash");
...
router.get("/:width(\\d+)x:height(\\d+)/:url(.*)", async function (ctx) {
    ...
    // Force a thumbnail until the web urls are requesting 1680x8400 instead of 0x0..  The thumbnail fixes image rotation.
    ...
    const isUpload = isUploadHash(url); // DQm...
    ...
    if (resizeRequest) {
        return await handleResizeRequest(ctx, targetWidth, targetHeight);
    }
    else {
        // A full size image
        return await handleFullSizeRequest(ctx);
    }
});
...
async function fetchImage(ctx, Bucket, Key, url, webBucketKey) {
    ...
    let imgResult;
    imgBuffer = await rp(opts);
    const ftype = fileType(imageBuffer);
    ctx.assert((!ftype || !/^image\/(gif|jpe?g|png)$/.test(ftype.mime)), 400,"Supported image formats are: gif, jpeg, and png" );
    imgResult = {Body: imageBuffer, ContentType: mime};
    ...
    if (imgResult) {
        ...
        ctx.logger.error('no imgResult from fetchImage()');
        return false;
    }
}


const handleFullSizeRequest = async function (ctx) {
    "use strict";
    const hasOriginal = !!(await headObject(originalKey).promise());

    // if we have an original, redirect to it
    if (hasOriginal) {
    ...
    // if we don't have an original:  1)fetch it 2)store it 3)redirect to it
    const imageResult = await fetchImage(ctx, Bucket, Key, url, webBucketKey);
    if (!imageResult) {
        ctx.logger.error('no imgResult from fetchImage()');
        ctx.assert(imageResult, 500, 'Error fetching image');
    }
...
const handleResizeRequest = async function (ctx, targetWidth, targetHeight) {
    "use strict";
    ...
    // if we already have a thumbnail cached, redirect to it
    if (hasThumbnail) {

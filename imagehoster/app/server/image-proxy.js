...
import {mhashEncode, sha1} from "app/server/hash";
import {missing, statusError} from "app/server/utils-koa";
import {s3, s3call, waitFor} from "app/server/amazon-bucket";
...
//const cloudfront_options = {keypairId: config.cloudfrontKeypairId, privateKey: config.cloudfrontPrivateKey};
const base_cloudfront_web_url = 'https://' + config.cloudfrontWebDomain + '/';
const base_cloudfront_thumb_url = 'https://' + config.cloudfrontThumbnailDomain + '/';
    ...
    if (resizeRequest) {
        ...
        if (fullSize) { // fullSize is used to show animations in the full-post size only
            ...
            if (imageHead && imageHead.ContentType === "image/gif") {
                ...
                let cf_url = base_cloudfront_web_url + Key;
